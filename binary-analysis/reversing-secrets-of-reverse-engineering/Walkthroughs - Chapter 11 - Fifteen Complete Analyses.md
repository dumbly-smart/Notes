# Chapter 11 — Fifteen Complete Authorized Protection-Solving Walkthroughs

> [!scope]
> Every solved protection is the self-authored Chapter 9 toy binary. This demonstrates the complete reversing process without bypassing third-party licensing.

## Complete target source (ground truth)
...
## Observed valid execution

```text
chapter09 evidence=5008423205
```

# Walkthrough 01 — Reproduce the serial relation

## Solving objective

Back-slice equality, recover FNV/XOR, calculate valid serial for MENTOR, and confirm accepted path.

## Ghidra stripped result
...
1. Start at accept/reject and backward-slice every contributing value.
2. Recover input normalization, widths, rotates, overflow, and formatting.
3. Trace outputs into later state/keys before considering a patch.
4. Implement the relation independently for the owned toy target.
5. Differential-test valid, invalid, and boundary cases against the original.
6. Record why the accepted path is valid rather than merely forcing control flow.

**Solved function:** `lab01_serial_checksum`, RVA `0x13e0`.

# Walkthrough 02 — Reproduce claim authenticator surrogate

## Solving objective

Recover exact covered bytes and expected value; explain clean reimplementation versus patch.
...
**Solved function:** `lab02_signed_claim`, RVA `0x1420`.

# Walkthrough 03 — Clone challenge response

## Solving objective

Translate eight rounds with 32-bit wrap and test several challenges.
...
**Solved function:** `lab03_challenge_response`, RVA `0x1440`.

# Walkthrough 04 — Recover device fingerprint

## Solving objective

Invert the data flow to compute stored fingerprint for controlled CPU/disk inputs.
...
**Solved function:** `lab04_device_binding`, RVA `0x1460`.

# Walkthrough 05 — Solve time-window constraints

## Solving objective

Derive accepted interval issued<=now<=issued+duration using subtraction-safe semantics.
...
**Solved function:** `lab05_expiry`, RVA `0x1480`.

# Walkthrough 06 — Satisfy feature-mask gate

## Solving objective

Prove requested must be subset of licensed and generate accepted/rejected masks.
...
**Solved function:** `lab06_features`, RVA `0x14a0`.

# Walkthrough 07 — Reconstruct replay precondition

## Solving objective

Choose a nonce absent from seen set and validate return without modifying code.
...
**Solved function:** `lab07_nonce_replay`, RVA `0x14b0`.

# Walkthrough 08 — Clone watermark calculation

## Solving objective

Recover per-user/build/copy relation with exact 32-bit operations.
...
**Solved function:** `lab08_watermark`, RVA `0x14e0`.

# Walkthrough 09 — Solve rollback tolerance

## Solving objective

Derive minimum acceptable now and identify overflow weakness in now+tolerance.
...
**Solved function:** `lab09_clock_rollback`, RVA `0x1500`.

# Walkthrough 10 — Solve quota inequality

## Solving objective

Derive requested<=limit-used after used<=limit.
...
**Solved function:** `lab10_usage_limit`, RVA `0x1510`.

# Walkthrough 11 — Forge only the self-owned toy receipt

## Solving objective

Recover body range, toy MAC, trailing product and create a matching local test receipt.
...
**Solved function:** `lab11_receipt`, RVA `0x1520`.

# Walkthrough 12 — Reproduce exposed token function

## Solving objective

Recover secret-dependent transform and explain why real secrets must stay off host.
...
**Solved function:** `lab12_token_oracle`, RVA `0x1560`.

# Walkthrough 13 — Map integrity boundary

## Solving objective

Recover exact range/expected digest and test inside/outside mutations.
...
**Solved function:** `lab13_code_integrity`, RVA `0x1570`.

# Walkthrough 14 — Extract runtime configuration

## Solving objective

Capture buffer after XOR stream and reproduce decode without modifying guard logic.
...
**Solved function:** `lab14_config_crypt`, RVA `0x1590`.

# Walkthrough 15 — Solve composite protection state

## Solving objective

Recover ordered serial/time/feature gates; show why success-branch-only patches are incomplete.
...
**Solved function:** `lab15_composite`, RVA `0x15c0`.

# Twenty Practice Questions

1. Patch versus keygen?
2. Why branch flip can fail?
3. How map runtime patch to file?
4. Why clone at fixed width?
5. How validate a clone?
6. What is clean-room solver?
7. Why avoid ripping code?
8. How choose checksum mutations?
9. What is plaintext oracle?
10. Why preserve normalization?
11. How solve subset mask?
12. How solve expiry?
13. What is bounded brute force?
14. Why inspect secondary threads?
15. What is manual export resolution clue?
16. What makes patch reversible?
17. How assess integrity dependency?
18. Why does runtime decrypt expose code?
19. What is lawful scope?
20. Mastery test?
...
1. Backward-slice the gate.
2. Track all downstream state and exact arithmetic.
3. **Answer:** Patch changes decision; keygen/reimplementation proves accepted relation.
4. Validate only on the owned teaching binary.

## 2. Solution

1. Backward-slice the gate.
2. Track all downstream state and exact arithmetic.
3. **Answer:** Derived state/key may be needed later.
4. Validate only on the owned teaching binary.
...
1. Backward-slice the gate.
2. Track all downstream state and exact arithmetic.
3. **Answer:** VA→RVA→section raw offset and whole instruction boundaries.
4. Validate only on the owned teaching binary.
...
1. Backward-slice the gate.
2. Track all downstream state and exact arithmetic.
3. **Answer:** Overflow/rotates define relation.
4. Validate only on the owned teaching binary.
...
1. Backward-slice the gate.
2. Track all downstream state and exact arithmetic.
3. **Answer:** Differential test accepted/rejected/boundary inputs.
4. Validate only on the owned teaching binary.
...
1. Backward-slice the gate.
2. Track all downstream state and exact arithmetic.
3. **Answer:** Independent implementation from observed contract, used only on owned challenge.
4. Validate only on the owned teaching binary.
...
1. Backward-slice the gate.
2. Track all downstream state and exact arithmetic.
3. **Answer:** Hidden globals/imports/relocations/ABI and legal/maintenance risks.
4. Validate only on the owned teaching binary.
...
1. Backward-slice the gate.
2. Track all downstream state and exact arithmetic.
3. **Answer:** Inside, outside, first, last, and relocation-sensitive bytes.
4. Validate only on the owned teaching binary.
...
1. Backward-slice the gate.
2. Track all downstream state and exact arithmetic.
3. **Answer:** Coherent code/data/format validates candidate key beyond a branch result.
4. Validate only on the owned teaching binary.
...
1. Backward-slice the gate.
2. Track all downstream state and exact arithmetic.
3. **Answer:** Different case/encoding/terminator bytes change validation.
4. Validate only on the owned teaching binary.
...
1. Backward-slice the gate.
2. Track all downstream state and exact arithmetic.
3. **Answer:** (requested&licensed)==requested.
4. Validate only on the owned teaching binary.
...
1. Backward-slice the gate.
2. Track all downstream state and exact arithmetic.
3. **Answer:** now>=issued and now-issued<=duration.
4. Validate only on the owned teaching binary.
...
1. Backward-slice the gate.
2. Track all downstream state and exact arithmetic.
3. **Answer:** Small authorized space plus strong low-false-positive oracle.
4. Validate only on the owned teaching binary.
...
1. Backward-slice the gate.
2. Track all downstream state and exact arithmetic.
3. **Answer:** They may produce state, timing, keys, or termination.
4. Validate only on the owned teaching binary.
...
1. Backward-slice the gate.
2. Track all downstream state and exact arithmetic.
3. **Answer:** Loader list, PE exports, hashes, indirect calls.
4. Validate only on the owned teaching binary.
...
1. Backward-slice the gate.
2. Track all downstream state and exact arithmetic.
3. **Answer:** Copy, recorded original bytes/hash, minimal change, regression.
4. Validate only on the owned teaching binary.
...
1. Backward-slice the gate.
2. Track all downstream state and exact arithmetic.
3. **Answer:** Trace checksum result beyond comparison into keys/state.
4. Validate only on the owned teaching binary.
...
1. Backward-slice the gate.
2. Track all downstream state and exact arithmetic.
3. **Answer:** Processor needs plaintext instructions before execution.
4. Validate only on the owned teaching binary.
...
1. Backward-slice the gate.
2. Track all downstream state and exact arithmetic.
3. **Answer:** Owned crackme or explicit authorization; no piracy.
4. Validate only on the owned teaching binary.
...
1. Backward-slice the gate.
2. Track all downstream state and exact arithmetic.
3. **Answer:** Solve all fifteen toy gates without source, document relations, and verify across unseen inputs.
4. Validate only on the owned teaching binary.


Return to [[Chapter 11 - Breaking Protections]].
